...
window.addEventListener("load", function() { //window.addEventListener("load" cuando carga la pagina
    ...
    // Botón de like
    function like() {
        $('.btn-like').unbind('click').click(function() { /*'.btn-like' la clase q esta en la vista entonces con el evento click(function..  unbind borra los eventos antiguos para q no salte like dislike varias veces yo solo quiero ua y otra y asi */
            console.log('like');
            $(this).addClass('btn-dislike').removeClass('btn-like'); /*this selecciona al objeto q le hice click addClass le agrego la clase btn-dislike y le saco la clase btn-like */
            $(this).attr('src', url + '/img/heart-red.png'); /*attr le cambio la etiqeta src por url + '/img/heart-red.png' le estoy poniendo el corazoncito rojo*/
            ...
            $.ajax({ /*creo el objeto json */
                url: 'http://localhost/blogLaravel/redsocial/public/like/' + $(this).data('id'), //el atributo id q toma desde la vista.. en realidad se llama data-id pero puedo cogerlo asi con data q es una funcion de javascrip q espera un id
